...
Expose the source-language families through the same named-suite mechanism used
by the earlier conformance families.

## Scope

- define named suites for `typescript`, `rust`, and `go`
...
1. a conformance manifest may declare named suites for `typescript`, `rust`,
   and `go`
2. each source-family suite references the representative `analysis`,
   `matching`, and `merge` roles
3. suite lookup and suite-name enumeration use the existing normalized manifest
   helpers unchanged
